...
define(__NAMESPACE__ . '\EBATNS_PARSEMODE_CALL', 1);
define(__NAMESPACE__ . '\EBATNS_PARSEMODE_NOTIFICATION', 2);
define(__NAMESPACE__ . '\EBATNS_PARSEMODE_CALLEXTENSION', 3);

define(__NAMESPACE__ . '\EBATNS_PSTATENOT_INITIAL', 0);
define(__NAMESPACE__ . '\EBATNS_PSTATENOT_HAS_SIGNATURE', 1);
define(__NAMESPACE__ . '\EBATNS_PSTATENOT_FOUND_ENVBODY', 2);
define(__NAMESPACE__ . '\EBATNS_PSTATENOT_IN_RESPONSE', 3);
...
class EbatNs_ResponseParser
{
    ...
    {
        if (! class_exists('\\' . __NAMESPACE__ . '\\' . $typeName))
        {
            $typeFileName = basename($typeName);
            require_once ucfirst($typeFileName) . '.php';
        }

        return (class_exists('\\' . __NAMESPACE__ . '\\' . $typeName));
    }
    ...
    protected function _makeValue ($typeName)
    {
        ...
        {
        	$nsTypeName = '\\' . __NAMESPACE__ . '\\' . $typeName;
    ...
    function decode ($responseName, $messageText, $parseMode = EBATNS_PARSEMODE_CALL, $responseTypeName = null)
    {
        ...
        if (! xml_parse($parser, $messageText, true))
        {
            ...
            $errResponse = new \InTradeSys\eBay\trading\EbatNs_ResponseError();
        ...
        if ($this->_responseObject === null && $responseName != 'errorMessage')
        {
        	...
        	{
        		if (stream_resolve_include_path(ucfirst($responseName) . '.php'))
        		{
	        		require_once ucfirst($responseName) . '.php';
	        		
	        		if (class_exists('\\' . __NAMESPACE__ . '\\' . $responseName))
	        		{
	        			
	        			$nsResponseName = '\\' . __NAMESPACE__ . '\\' . $responseName;
	        			
	        			$responseObject = new $nsResponseName();
        ...
        if ((($this->_parseMode == EBATNS_PARSEMODE_CALL || $this->_parseMode == EBATNS_PARSEMODE_CALLEXTENSION) 
            ...
            else
            {
                ...
                else
                {
                    ...
                    $typeInfo = $this->_typeMap[strtolower(end(explode('\\', get_class($parent))))];
    ...
    function _endElement ($parser, $name)
    {
        ...
        if ($current)
            $typeInfoCurrent = $this->_typeMap[strtolower(end(explode('\\', get_class($current))))];
        if ($parent)
            $typeInfoParent = $this->_typeMap[strtolower(end(explode('\\', get_class($parent))))];
